...
cmbn <- dplyr::select(quant.tbl, r1, r2) %>% dplyr::distinct()
cmbn.class <- dplyr::select(quant.tbl, class1, class2) %>% dplyr::distinct() %>%
              dplyr::mutate(class1=sanitize(class1), class2=sanitize(class2))
...
if (is.single.comparision) {
  title.comp <- paste(cmbn.class$class2[1],"/",cmbn.class$class1[1])
} else {
  title.comp <- NULL
}
...
n.quant <- dplyr::group_by(quant.tbl, class1, class2) %>% summarize(n_na = sum(is.na(lratio)))
...
proteins.n.group <- dplyr::select(quant.tbl, ac, group) %>% dplyr::distinct() %>%
                    dplyr::arrange(group)
print_protein_grp_tbl("protein-groups.tex",proteins.n.group$ac,protein.group)
