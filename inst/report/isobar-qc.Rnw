...
<<echo=FALSE,results=tex>>=
reporter.tags <- sanitize(reporterTagNames(ibspectra))
...
if (!is.null(ratiodistr)) {
  ...
  if (!is.null(attr(ratiodistr,"method"))) {
     ...
     classLabels <- sanitize(attr(ratiodistr,"cl"))
     t <- table(classLabels)
     tagNames <- sanitize(attr(ratiodistr,"tagNames"))
...
gg <- grid.arrange(g,
tableGrob(dplyr::group_by_(quant.tbl, c('channels', 'class2', 'class1')[c('channels','class2','class1') %in% colnames(quant.tbl)]) %>%
          dplyr::summarize(`number of ratios`=sum(!is.na(lratio)),
                           `significantly\ndownregulated`=sum(is.significant & lratio<0, na.rm=TRUE),
                           `significantly\nupregulated`=sum(is.significant & lratio>0, na.rm=TRUE)),
          theme=ttheme_default(core=list(fg_params=list(fontsize=9)),
...
quant.tbl <- dplyr::group_by(quant.tbl, sign.string) %>% mutate(n_same = n()) %>% ungroup() %>%
             dplyr::mutate(sign = paste0(sign.string,": ",n_same))
